...
#indexNames = df2[(df2['age'] == None) & (df2['age'] >= 80) & (df2['age'] <= 10)].index
#df2 = df2.drop(indexNames, inplace=True)
df2['age'] = df2['age'].replace(np.nan, 1)
df2.drop(df2[df2['age'] >= 80].index, inplace = True)
df2.drop(df2[df2['age'] < 20].index, inplace = True)
#df2.drop(df2[df2['age'] is None].index, inplace = True)
print(df2)
exit()
#df2.to_csv("bda_final_age.csv", index=False, encoding="utf-8_sig")
...
for i in range(len(df1)):
    ...
    if level in level_amount:
        ...
        level_amount[level] = [set(id), amount, good_quantity, age]
    count += 1
    #print(level_amount[level][3])
print(count)

for i in level_amount:
    level_amount[i] = [level_amount[i][3]/len(level_amount[i][0])]
